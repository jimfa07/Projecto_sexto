...
PROVEEDORES = ["LIRIS SA", "Gallina 1", "Monze Anzules", "Medina", "Otros"] # Añadido "Otros" para flexibilidad
...
AGENCIAS = [
    ...
    "Banco Bolivariano", "Otros" # Añadido "Otros"
]
...
COLUMNS_DEPOSITS = ["N", "Fecha", "Empresa", "Agencia", "Monto", "Documento"] # "N" al inicio para consistencia
...
def recalculate_accumulated_balances():
    ...
    if not df_deposits.empty:
        ...
        # Agrupar por Fecha y Empresa (Proveedor) para sumar los depósitos
        ...
        # Usamos un merge 'left' para mantener todas las filas de df_data_operaciones
    ...
    if not df_notes.empty:
        ...
        # Las notas de débito reducen el saldo, por eso se restan
    ...
    full_daily_balances = full_daily_balances.sort_values("Fecha").reset_index(drop=True)

    # Calcular Saldo Acumulado de las operaciones
    # Se inicializa con el saldo inicial para el primer día de operación
    # Y luego se suman los saldos diarios ajustados.
    full_daily_balances["Saldo Acumulado"] = full_daily_balances["SaldoDiarioAjustado"].cumsum() + INITIAL_ACCUMULATED_BALANCE
    
    # Asegurar que todas las columnas de df_data_operaciones estén presentes y en el orden correcto
    df_data_operaciones = df_data_operaciones[COLUMNS_DATA] # Reordenar y asegurar columnas

    # Fusionar los saldos diarios y acumulados de vuelta al df_data_operaciones
    # Se hace un merge por fecha para aplicar los saldos diarios y acumulados calculados por día
    # Esto es crucial para que cada registro dentro de un día refleje el saldo de ese día.
    df_data_operaciones_merged = pd.merge(
        df_data_operaciones.drop(columns=["Saldo diario", "Saldo Acumulado"], errors='ignore'),
        full_daily_balances[["Fecha", "SaldoDiarioAjustado", "Saldo Acumulado"]],
        on="Fecha",
        how="left"
    )
    df_data_operaciones_merged.rename(columns={"SaldoDiarioAjustado": "Saldo diario_temp"}, inplace=True)
    df_data_operaciones_merged["Saldo diario"] = df_data_operaciones_merged["Saldo diario_temp"].fillna(0)
    df_data_operaciones_merged.drop(columns=["Saldo diario_temp"], inplace=True, errors='ignore')

    # Si hay fechas en df_data_operaciones que no tienen entrada en full_daily_balances (ej. sin movimientos de saldo),
    # su saldo acumulado debe ser el del día anterior o el inicial.
    # Para esto, se debe re-calcular el saldo acumulado de forma iterativa, asegurando que se propague correctamente.
    ...
    if not df_initial_balance.empty:
        ...
        df_data = pd.concat([df_initial_balance, df_data_operaciones_merged], ignore_index=True)
    else:
        df_data = df_data_operaciones_merged
    ...
    # Es crucial que el orden sea correcto para el cálculo iterativo del saldo acumulado.
    df_data["N_numeric"] = pd.to_numeric(df_data["N"], errors='coerce').fillna(0)
    df_data = df_data.sort_values(by=["Fecha", "N_numeric"], ascending=[True, True]).reset_index(drop=True)
    df_data.drop(columns=["N_numeric"], inplace=True)

    # RECALCULAR EL SALDO ACUMULADO DE MANERA ITERATIVA Y ROBUSTA
    # Este es el método más seguro para un saldo acumulado correcto
    df_data["Saldo Acumulado"] = 0.0 # Resetear para el cálculo
    current_accumulated_balance = INITIAL_ACCUMULATED_BALANCE
    
    # Agrupar saldos diarios y notas de débito por fecha para obtener un "neto" diario
    daily_net_change = df_data[df_data["Proveedor"] != "BALANCE_INICIAL"].groupby("Fecha")["Saldo diario"].sum()

    # Si hay notas de débito, ajustar el cambio diario
    if not df_notes.empty:
        df_notes["Descuento real"] = pd.to_numeric(df_notes["Descuento real"], errors='coerce').fillna(0)
        notes_daily_sum = df_notes.groupby("Fecha")["Descuento real"].sum()
        daily_net_change = daily_net_change.add(-notes_daily_sum, fill_value=0) # Restar notas de débito al cambio diario

    # Crear una serie de fechas únicas y ordenadas de todo el historial
    all_dates = sorted(df_data["Fecha"].unique())
    
    # Diccionario para mapear fecha a saldo acumulado al final del día
    accumulated_balance_per_day = {datetime(1900, 1, 1).date(): INITIAL_ACCUMULATED_BALANCE} # Punto de partida

    for current_date in all_dates:
        if current_date == datetime(1900, 1, 1).date():
            continue # Ya handled por el balance inicial

        # Obtener el saldo acumulado del día anterior
        previous_date = None
        for d in sorted(accumulated_balance_per_day.keys()):
            if d < current_date:
                previous_date = d
            else:
                break
        
        balance_from_previous_day = accumulated_balance_per_day.get(previous_date, INITIAL_ACCUMULATED_BALANCE)

        # Sumar el cambio neto del día actual (si existe)
        daily_change = daily_net_change.get(current_date, 0.0)
        current_accumulated_balance_for_day = balance_from_previous_day + daily_change
        accumulated_balance_per_day[current_date] = current_accumulated_balance_for_day

    # Asignar el saldo acumulado a cada fila basado en la fecha de la fila
    for index, row in df_data.iterrows():
        if row["Proveedor"] == "BALANCE_INICIAL":
            df_data.loc[index, "Saldo Acumulado"] = INITIAL_ACCUMULATED_BALANCE
            df_data.loc[index, "Saldo diario"] = 0.0 # Balance inicial no tiene saldo diario
        else:
            df_data.loc[index, "Saldo Acumulado"] = accumulated_balance_per_day.get(row["Fecha"], INITIAL_ACCUMULATED_BALANCE)
            # El saldo diario para cada registro de operación ya se calculó arriba.
            # Aquí solo se asegura que no se pierda.
            # Ya se calculó y se fusionó en df_data_operaciones_merged
            # df_data.loc[index, "Saldo diario"] = df_data.loc[index, "Monto Deposito"] - df_data.loc[index, "Total ($)"]
...
def get_next_n(df, current_date):
    ...
    if not df_filtered.empty:
        # Asegurarse de que 'N' sea string y luego convertir a int para el máximo
        df_filtered["N_int"] = df_filtered["N"].apply(lambda x: int(x) if isinstance(x, str) and x.isdigit() else 0)
        max_n_global = df_filtered["N_int"].max()
...
def add_deposit_record(fecha_d, empresa, agencia, monto):
    ...
    st.session_state.df = st.session_state.df[COLUMNS_DEPOSITS] # Asegurar orden de columnas
...
def edit_deposit_record(index_to_edit, updated_data):
    ...
    try:
        ...
        current_df.loc[index_to_edit, "Documento"] = "Deposito" if "Cajero" in str(agencia) else "Transferencia" # str() para seguridad
...
def add_supplier_record(fecha, proveedor, cantidad, peso_salida, peso_entrada, tipo_documento, gavetas, precio_unitario):
    ...
    st.session_state.data = st.session_state.data[COLUMNS_DATA] # Asegurar orden de columnas
...
def add_debit_note(fecha_nota, descuento, descuento_real):
    ...
    st.session_state.notas = st.session_state.notas[COLUMNS_DEBIT_NOTES] # Asegurar orden de columnas
...
def render_delete_deposit_section():
    ...
    if not st.session_state.df.empty:
        ...
        df_display_deposits["Display"] = df_display_deposits.apply(
            lambda row: f"ID {row.name} - {row['Fecha']} - {row['Empresa']} - ${row['Monto']:.2f}", axis=1
        )
        ...
        if deposito_seleccionado_info:
            try:
                index_to_delete = int(deposito_seleccionado_info.split(' - ')[0].replace("ID ", ""))
            except ValueError:
                index_to_delete = None
...
def render_edit_deposit_section():
    ...
    if not st.session_state.df.empty:
        ...
        if deposito_seleccionado_info:
            try:
                index_to_edit = int(deposito_seleccionado_info.split(' - ')[0].replace("ID ", ""))
            except ValueError:
                index_to_edit = None
...
def render_delete_debit_note_section():
    ...
    if not st.session_state.notas.empty:
        ...
        df_display_notes["Display"] = df_display_notes.apply(
            lambda row: f"ID {row.name} - {row['Fecha']} - Descuento real: ${row['Descuento real']:.2f}", axis=1
        )
        ...
        if nota_seleccionada_info:
            try:
                index_to_delete = int(nota_seleccionada_info.split(' - ')[0].replace("ID ", ""))
            except ValueError:
                index_to_delete = None
...
def render_edit_debit_note_section():
    ...
    if not st.session_state.notas.empty:
        ...
        if nota_seleccionada_info:
            try:
                index_to_edit = int(nota_seleccionada_info.split(' - ')[0].replace("ID ", ""))
            except ValueError:
                index_to_edit = None
...
def display_formatted_dataframe(df_source, title, columns_to_format=None, key_suffix="", editable_cols=None):
    ...
    # El KeyError en "edited_rows" ocurre si el diccionario no tiene esa clave.
    # Usar .get() con un valor por defecto seguro (un diccionario vacío) es la forma correcta.
    if st.session_state.get(f"editable_df_{key_suffix}") and st.session_state[f"editable_df_{key_suffix}"].get("edited_rows"):
            ...
            try:
                ...
                for idx_str, changes in df_updated_rows.items():
                    ...
                    for col, value in changes.items():
                        ...
                        # Es importante usar el tipo original del df_source, no del df_display (que es todo string)
                        original_type_series = df_source[col]
                        
                        if pd.api.types.is_datetime64_any_dtype(original_type_series) or col == "Fecha":
                            original_df_to_update.loc[idx, col] = pd.to_datetime(value).date()
                        elif pd.api.types.is_integer_dtype(original_type_series) or col in ["Cantidad", "Cantidad de gavetas", "N"]:
                            original_df_to_update.loc[idx, col] = int(pd.to_numeric(value, errors='coerce').fillna(0))
                        elif pd.api.types.is_float_dtype(original_type_series) or col in ["Peso Salida (kg)", "Peso Entrada (kg)", "Precio Unitario ($)", "Monto", "Descuento", "Descuento real"]:
                            original_df_to_update.loc[idx, col] = float(pd.to_numeric(value, errors='coerce').fillna(0.0))
                        else:
...
def render_tables_and_download():
    ...
    if not df_display_data.empty:
        ...
        df_display_data_for_del["Display"] = df_display_data_for_del.apply(
            lambda row: f"ID {row.name} - {row['Fecha']} - {row['Proveedor']} - ${row['Total ($)']:.2f}"
            if pd.notna(row["Total ($)"]) else f"ID {row.name} - {row['Fecha']} - {row['Proveedor']} - Sin total",
            axis=1
        )
        ...
        if not df_display_data_for_del.empty:
            ...
            if registro_seleccionado_info:
                try:
                    index_to_delete_record = int(registro_seleccionado_info.split(' - ')[0].replace("ID ", ""))
                except ValueError:
                    index_to_delete_record = None
    ...
    @st.cache_data
    def convertir_excel(df_data_export, df_deposits_export, df_notes_export):
        output = BytesIO()
        with pd.ExcelWriter(output, engine="openpyxl") as writer:
            df_data_export.to_excel(writer, sheet_name="Registros", index=False)
            df_deposits_export.to_excel(writer, sheet_name="Depositos", index=False)
            df_notes_export.to_excel(writer, sheet_name="Notas de Debito", index=False)
        output.seek(0)
        return output

    if not st.session_state.data.empty or not st.session_state.df.empty or not st.session_state.notas.empty:
        df_data_filtered = st.session_state.data[st.session_state.data["Proveedor"] != "BALANCE_INICIAL"].copy()
        
        # Eliminar columnas temporales de "Display" antes de exportar
        df_deposits_export = st.session_state.df.copy()
        if "Display" in df_deposits_export.columns:
            df_deposits_export = df_deposits_export.drop(columns=["Display"])
        
        df_notes_export = st.session_state.notas.copy()
        if "Display" in df_notes_export.columns:
            df_notes_export = df_notes_export.drop(columns=["Display"])
        
        st.download_button(
            label="⬇️ Descargar Todos los Datos en Excel",
            data=convertir_excel(df_data_filtered, df_deposits_export, df_notes_export),
    ...
    if columns_to_format:
        for col in columns_to_format:
            if col in df_pdf.columns:
                ...
                df_pdf[col] = df_pdf[col].apply(lambda x: f"${x:,.2f}" if pd.notna(x) else "N/A")
...
def render_weekly_report():
    ...
    if not df.empty:
        ...
        if not df.empty:
            ...
            # La semana comienza en lunes (isocalendar().weekday == 1)
            # Para obtener el inicio de la semana (lunes)
            start_of_week = today - pd.Timedelta(days=today.weekday())
            # Para obtener el final de la semana (domingo)
            end_of_week = start_of_week + pd.Timedelta(days=6)
            
            df_semana = df[(df["Fecha"] >= start_of_week) & (df["Fecha"] <= end_of_week)].copy()
            df_semana = df_semana.sort_values(by="Fecha", ascending=True)

            if not df_semana.empty:
                st.subheader(f"Registros de la Semana del {start_of_week.strftime('%Y-%m-%d')} al {end_of_week.strftime('%Y-%m-%d')}")
                display_formatted_dataframe(
                    df_semana, 
                    "Registros de la Semana", # Título genérico para no duplicar subheader
                    columns_to_format=["Total ($)", "Monto Deposito", "Saldo diario", "Saldo Acumulado"],
                    key_suffix="weekly_report_display"
                )
                content_elements.append(Paragraph(f"<b>Registros de la Semana del {start_of_week.strftime('%Y-%m-%d')} al {end_of_week.strftime('%Y-%m-%d')}</b>", getSampleStyleSheet()['h2']))
                ...
                st.info(f"No hay datos para la semana actual (del {start_of_week.strftime('%Y-%m-%d')} al {end_of_week.strftime('%Y-%m-%d')}).")
                content_elements.append(Paragraph(f"No hay datos para la semana actual (del {start_of_week.strftime('%Y-%m-%d')} al {end_of_week.strftime('%Y-%m-%d')}).", getSampleStyleSheet()['Normal']))
        else:
...
def render_monthly_report():
    ...
    if not df.empty:
        ...
        if not df.empty:
            ...
            df_mes = df[(df["Fecha"].dt.month == mes_actual) & (df["Fecha"].dt.year == año_actual)].copy()
            ...
            if not df_mes.empty:
                ...
                display_formatted_dataframe(
                    df_mes, 
                    "Registros del Mes", # Título genérico para no duplicar subheader
                    columns_to_format=["Total ($)", "Monto Deposito", "Saldo diario", "Saldo Acumulado"],
